    ...
    return function transformer(tree: Root) {
        visit(tree, 'element', function (node: Element, index, parent) {
            ...
            while (
                ...
                whitespace(node.children[headIndex] as Nodes | string)
                ) {
                headIndex++
            }
            ...
            if (end + 1 === text.value.length) {
                ...
                if (next) {
                    ...
                    while (
                        ...
                        whitespace(node.children[headIndex + 1] as Nodes | string)
                        ) {
                        skipped = true
                        headIndex++
                    }
            ...
            // Get alert children and add mb-0 to the last element
            const alertChildren = node.children.slice(headIndex);

            // Find the last non-whitespace element
            let lastElementIndex = alertChildren.length - 1;
            while (lastElementIndex >= 0 && whitespace(alertChildren[lastElementIndex] as Nodes | string)) {
                lastElementIndex--;
            }
            ...
            parent.children[index] = {
                ...
                tagName: 'div',
                properties: {
                    className: ['alert', 'alert-' + alterName, extraClassName],
                    role: "alert"
                },
                children: [
                    {
                        ...
                        children: [
                            {
                                ...
                                tagName: 'svg',
                                properties: {
                                    className: ['octicon', icon.name, 'me-2'],
                                    viewBox: '0 0 16 16',
                                    version: '1.1',
                                    width: '16',
                                    height: '16',
